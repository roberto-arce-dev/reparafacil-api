...
Clientes y técnicos de reparaciones
...
```env
# MongoDB
MONGODB_URI=mongodb://localhost:27017/reparafacil_db
...
```env
MONGODB_URI=mongodb+srv://usuario:password@cluster.mongodb.net/reparafacil_db
...
El servidor estará disponible en: `http://localhost:3014`
...
#### Listar todos los perfiles (Admin)
```bash
GET /api/tecnico-profile
Authorization: Bearer {token_admin}
```

#### Obtener perfil por userId (Admin)
```bash
GET /api/tecnico-profile/{userId}
Authorization: Bearer {token_admin}
```
...
La imagen **NO es requerida** al crear un servicio. Puedes:
- ✅ Crear el servicio SIN imagen
- ✅ Agregar la imagen DESPUÉS usando el endpoint de upload

### 🎯 Flujo Recomendado (Paso a Paso)

#### Paso 1: Crear el Servicio sin imagen
...
**💡 Nota:** Guarda el `_id` del servicio creado, lo necesitarás para subir la imagen.

---

#### Paso 2: Subir imagen al Servicio

```bash
POST /api/servicio/507f1f77bcf86cd799439011/upload-image
Authorization: Bearer {{token}}
...
4. **Ir a:** `Servicio` → `Create Servicio`
...
8. **Copiar** el `_id` del servicio creado

---

#### Paso 2: Subir Imagen

1. **Ir a:** `Upload por Entidad` → `Upload Servicio Image`
2. **Reemplazar** `{{id}}` en la URL con el ID copiado:
   ```
   http://localhost:3014/api/servicio/507f1f77bcf86cd799439011/upload-image
   ```
...
**✅ ¡Listo!** Tu servicio ahora tiene imagen.
...
**Imagen original:**
```
http://localhost:3014/uploads/1700000000000-imagen.jpg
```

**Thumbnail (miniatura):**
```
http://localhost:3014/uploads/thumbnails/thumb-1700000000000-imagen.jpg
```
...
TOKEN=$(curl -X POST http://localhost:3014/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"productor@example.com","password":"password123"}' \
  | jq -r '.access_token')

# 2. Crear Servicio
PRODUCTO_ID=$(curl -X POST http://localhost:3014/api/servicio \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{   "nombre": "Servicio de ejemplo",   "descripcion": "Descripción del servicio",   "disponible": true }' \
  | jq -r '.data._id')

echo "Servicio creado con ID: $PRODUCTO_ID"

# 3. Subir imagen
curl -X POST http://localhost:3014/api/servicio/$PRODUCTO_ID/upload-image \
  -H "Authorization: Bearer $TOKEN" \
...
- La URL debe empezar con `http://localhost:3014/uploads/`
...
```bash
curl -X GET http://localhost:3014/api/cliente-profile/me \
...
```bash
curl -X PUT http://localhost:3014/api/cliente-profile/me \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
**Archivo:** `reparafacil-api.postman_collection.json`
...
│   │
│   ├── tecnico-profile/  # Profile TECNICO
│   │   ├── dto/
│   │   ├── schemas/
│   │   ├── tecnico-profile.controller.ts
│   │   ├── tecnico-profile.service.ts
│   │   └── tecnico-profile.module.ts
│   │
...
├── reparafacil-api.postman_collection.json
└── package.json
```
